...
function computeOptimalBodyWidth(spans, rs) {
  ...
  try {
    if (hasExplicit) {
      ...
      const optimalW = Math.min(Math.ceil(maxW) + padding, canvasW);
      ...
      if (fullLines <= 1) {
        ...
        const optimalW = Math.min(textW + padding, canvasW);
        return { bodyW: optimalW, bodyX: Math.round((canvasW - optimalW) / 2) };
      }
      ...
      inner.style.whiteSpace = 'normal';
      inner.innerHTML        = spansToHtmlPreview(spans);
      let lo = 200, hi = canvasW, best = canvasW;
      while (lo <= hi) {
      ...
      const optimalW = Math.min(best + padding, canvasW);
      return { bodyW: optimalW, bodyX: Math.round((canvasW - optimalW) / 2) };
    }
  } finally {
...
function renderConfigPanel(panel) {
      ...
      <div class="settings-section">
        ...
        <div class="settings-row" style="margin-bottom:12px">
          ...
          <div class="output-mode-group">
            ${[['local','Save to folder'],['download','Download link'],['deliver','Deliver to Pro7'],['ask','Ask every time']].map(([val, lbl]) => `
              <label class="output-mode-opt">
                <input type="radio" name="outputMode" value="${val}" ${(cfg.outputMode || 'local') === val ? 'checked' : ''}>
                ${esc(lbl)}
              </label>
            `).join('')}
          </div>
        </div>
        <div class="settings-row" id="output-folder-row" style="${(cfg.outputMode || 'local') === 'local' ? '' : 'display:none'}">
          <label>Folder</label>
          <div class="folder-row" style="flex:1">
            <input type="text" id="cfg-outputFolder" value="${esc(cfg.outputFolder)}" placeholder="~/Documents/ProPresenter" readonly>
            <button class="btn-browse" id="btn-browse-folder">Browse…</button>
          </div>
        </div>
      </div>
  ...
  document.querySelectorAll('input[name="outputMode"]').forEach(r => {
    r.addEventListener('change', e => {
      cfg.outputMode = e.target.value;
      const folderRow = document.getElementById('output-folder-row');
      if (folderRow) folderRow.style.display = cfg.outputMode === 'local' ? '' : 'none';
      saveState();
    });
  });
  ...
  document.getElementById('btn-browse-folder')?.addEventListener('click', async () => {
    try {
      ...
      if (data.ok && data.folder) {
        cfg.outputFolder = data.folder;
        document.getElementById('cfg-outputFolder').value = data.folder;
        saveState();
      }
    } catch (_) {}
  });
...
function lyTable(rows, scheme, dis) {
  ...
  const bodyRows = rows.map(row => {
    if (row.type === 'head') {
      return `<tr class="ly-section-head"><td colspan="5">${esc(row.label)}</td></tr>`;
    }
  ...
  return `<table class="ly-table">
    <thead><tr>
      <th></th><th>X</th><th>Y</th><th>W</th><th>H</th>
    </tr></thead>
...
function renderStylePanel(panel) {
  ...
  ['bodyFont', 'propBodyFont', 'boldFont', 'titleFont', 'startEndFont'].forEach(field => {
      ...
      <div class="bullet-list" id="bullet-list">
        ${(slide.bullets || ['']).map((b, i) => `
          <div class="bullet-row" data-bullet-idx="${i}">
            ...
            <input type="text" class="bullet-input" spellcheck="true" data-idx="${i}" value="${esc(b)}" placeholder="Bullet ${i + 1}">
...
function attachFormHandlers(slide) {
      ...
      attachRichEditor(`f-body-${idx}`, spans => {
        ...
        if (slide.fitWidth) {
          ...
          _fitTimer = setTimeout(() => {
            const s = (state.schemes || {})[state.activeScheme || ''] || {};
  ...
  // Bullet inputs
  document.querySelectorAll('.bullet-input').forEach(inp => {
    inp.addEventListener('input', () => {
      const idx = Number(inp.dataset.idx);
      if (!slide.bullets) slide.bullets = [];
      slide.bullets[idx] = inp.value;
      // Auto-sync label from first bullet
      if (idx === 0 && !slide._labelManual) {
        slide.label = inp.value;
...
function preflightWarnings() {
  ...
  for (const slide of state.slides) {
    ...
    if (slide.type === 'point' && slide.mode === 'revealing') {
      if (!(slide.bullets || []).some(b => b?.trim()))
...
async function generate() {
  ...
  const outputMode = state.config.outputMode || 'local';
  ...
  await runGenerate(outputMode === 'download', btn, outputMode === 'deliver');
}
...
async function runSchemeTest(scheme) {
  ...
  const outputMode   = state.config.outputMode === 'ask' ? 'local' : (state.config.outputMode || 'local');
  const downloadMode = outputMode === 'download';
  const deliverMode  = outputMode === 'deliver';

  const TEST_SPEC = {
    name: testName,
    outputFolder:        state.config.outputFolder || '',
    ...(downloadMode && { downloadMode: true }),
    ...(deliverMode  && { deliverMode:  true }),
    includeResponseCard: true,
  ...
  try {
    ...
    if (data.downloadMode) triggerDownloads(data);
...
async function runGenerate(downloadMode, btn, deliverMode = false) {
  ...
  // Props write to Configuration/Props which Pro7 overwrites on quit.
  // Require Pro7 to be closed before delivering so our write sticks.
  if (deliverMode) {
    try {
    ...
    } catch (_) { /* can't check — allow delivery anyway */ }
  }

  btn.disabled = true;
  btn.textContent = deliverMode ? 'Delivering…' : 'Generating…';

  const spec     = buildSpec();
  const fileName = spec.name;
  if (downloadMode) spec.downloadMode = true;
  if (deliverMode) spec.deliverMode = true;
  ...
  try {
    if (deliverMode) updateDeliveryStep(0, false);
    const res  = await fetch('/api/generate', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (data.ok) {
      ...
      if (data.downloadMode) triggerDownloads(data);
      if (!data.downloadMode) addGenHistoryEntry(data);
      showGenerateModal(data, spec);
    } else {
      if (deliverMode) hideDeliveryOverlay();
...
function showProRunningModal() {
  ...
  body.innerHTML = `
    <p style="font-size:13px;color:var(--muted);line-height:1.5;margin-bottom:14px">
      ...
      Close ProPresenter, then deliver — it'll be ready to open when you're done.
    </p>
    ...
    overlay.innerHTML = `
      <div class="warn-modal">
        ...
        <div class="warn-actions">
          ...
          <button class="warn-btn-ok">Generate Anyway</button>
        </div>
      </div>`;
...
function renderGenHistory() {
  ...
  if (!h.length) {
    panel.innerHTML = `<div class="gen-history-empty">No generations yet</div>`;
    return;
  }
  const fmt = iso => {
    ...
    ${h.map(e => `
      <div class="gen-history-item" data-path="${esc(e.path)}">
        ...
        <div class="ghi-info">
          ...
          <div class="ghi-meta">${e.delivered ? '⬆ Delivered · ' : ''}${e.sizeKB ? e.sizeKB + ' KB · ' : ''}${fmt(e.date)}</div>
        </div>
    ...
    if (slide.type === 'scripture') {
      ...
      if (slide.mode === 'revealing') {
        bodyHtml = (slide.bullets || []).map((b, i) =>
          `${i < (slide.bullets.length - 1) ? '' : '<strong>'} ${esc(b)}${i < (slide.bullets.length - 1) ? '' : '</strong>'}`
        ).join('<br>');
      } else {
...
const DECKS_KEY = 'deckpro_saved_decks';
...
function saveDecks(decks) {
  try { localStorage.setItem(DECKS_KEY, JSON.stringify(decks)); } catch (_) {}
}
...
function loadDeckState(id) {
  ...
  if (!deck) return;
  state = deck.state;
  state.currentDeckId = id; // ensure it's set even in older saved decks
...
function newDeck({ series = '', title = '', date = '', schemeId = null } = {}) {
  ...
  s.activeSchemeId       = schemeId || state.activeSchemeId;
  state = s;
  _undoStack.length = 0; _redoStack.length = 0;
  saveState();
  render();
...
function renderDecksList() {
  ...
  if (_deckSort === 'series') {
    decks = [...decks].sort((a, b) => {
      ...
      return sc !== 0 ? sc : (b.date || '').localeCompare(a.date || '');
    });
  } else {
    decks = [...decks].sort((a, b) =>
  ...
  if (!decks.length) {
    list.innerHTML = `
      <div class="deck-empty">
        ...
        No decks yet. Hit <strong>+ New Deck</strong> to start one.
      </div>`;
    return;
  }

  const curId = state.currentDeckId;

  // Draft entry — shown at top when working state isn't in the library yet
  const draftHtml = !curId ? `
  ...
  list.innerHTML = draftHtml + decks.map(deck => {
    const isCurrent = deck.id === curId;
    const isEditing = deck.id === _editingDeckId;
    const series    = deck.series || '';
    const title     = deck.title  || (deck.name && !deck.series ? deck.name : '');
    const dateLabel = deckDateLabel(deck.date);
    const meta      = [dateLabel, deck.slideCount != null ? `${deck.slideCount} slides` : ''].filter(Boolean).join(' · ');

    if (isEditing) {
      return `
        <div class="deck-item deck-item--editing" data-id="${deck.id}">
          <div class="deck-edit-fields">
            <input class="deck-edit-input" id="de-series" placeholder="Series" value="${esc(series)}" spellcheck="true">
            <input class="deck-edit-input" id="de-title"  placeholder="Title"  value="${esc(title)}"  spellcheck="true">
            ...
            <label class="deck-edit-qr-wrap" title="QR Code">
              <div class="toggle${deck.state?.config?.qrCode ? ' on' : ''}" id="de-qr-toggle"></div>
              QR
            </label>
          </div>
          <div class="deck-edit-actions">
    ...
    return `
      <div class="deck-item${isCurrent ? ' deck-item--current' : ''}" data-id="${deck.id}">
        <div class="deck-item-info">
          ${isCurrent ? '<span class="deck-current-badge">Current</span>' : ''}
          ...
          ${meta ? `<div class="deck-item-meta">${meta}</div>` : ''}
        </div>
        <div class="deck-item-actions">
          <button class="btn-deck-edit"   data-id="${deck.id}">Edit</button>
          ${!isCurrent ? `<button class="btn-deck-load" data-id="${deck.id}">Load</button>` : ''}
          <button class="btn-deck-delete" data-id="${deck.id}" title="Delete">×</button>
        </div>
      </div>`;
  }).join('');
}
...
function initDecks() {
  ...
  document.getElementById('btn-nd-create')?.addEventListener('click', () => {
    ...
    newDeck({ series, title, date, schemeId });
    document.getElementById('new-deck-form')?.classList.add('hidden');
  ...
  // List interactions
  list?.addEventListener('click', e => {
    const editBtn    = e.target.closest('.btn-deck-edit');
    const saveEdit   = e.target.closest('.btn-deck-edit-save');
    const cancelEdit = e.target.closest('.btn-deck-edit-cancel');
    const loadBtn    = e.target.closest('.btn-deck-load');
    const delBtn     = e.target.closest('.btn-deck-delete');
    const qrToggle   = e.target.closest('#de-qr-toggle');

    if (qrToggle) { qrToggle.classList.toggle('on'); return; }

    if (editBtn) {
      _editingDeckId = _editingDeckId === editBtn.dataset.id ? null : editBtn.dataset.id;
      renderDecksList();
    }
    if (saveEdit) {
      ...
      const idx    = decks.findIndex(d => d.id === id);
      if (idx >= 0) {
        decks[idx].series = series;
        decks[idx].title  = title;
        decks[idx].date   = date;
        if (decks[idx].state?.config) decks[idx].state.config.qrCode = qrOn;
        saveDecks(decks);
    ...
    if (loadBtn) {
      loadDeckState(loadBtn.dataset.id);
      _editingDeckId = null;
      modal.classList.remove('open');
    }
    if (delBtn) {
      if (_editingDeckId === delBtn.dataset.id) _editingDeckId = null;
      deleteDeck(delBtn.dataset.id);
      renderDecksList();
    }
  });
}
...
const HELP_SECTIONS = [
  {
    ...
    html: `
      ...
      <p>DeckPro builds ProPresenter 7 slide decks for Canvas Church messages. Fill in your slides, hit Generate, and a ready-to-use <code>.pro</code> file drops into your ProPresenter library.</p>
      <h4>Workflow</h4>
      <ul>
        <li>Set the <strong>Series</strong>, <strong>Title</strong>, and <strong>Date</strong> in the header</li>
        <li>Add slides using the <strong>Add Element</strong> buttons in the sidebar</li>
        <li>Fill in content for each slide in the main panel</li>
        <li>Hit <strong>Generate</strong> to produce the .pro file</li>
      </ul>
  ...
  },
  {
    ...
    html: `
      <h4>Scripture</h4>
      <p>A verse or passage slide. Enter the reference and body text, or use the Bible lookup to auto-fill. The reference bar appears on the ATEM lower-third. Each scripture slide creates a matching prop file.</p>
      <h4>Point</h4>
      <p>A message point slide. Two modes:</p>
      <ul>
        <li><strong>Single</strong> — one prop that stays static while the slide is shown</li>
        <li><strong>Revealing</strong> — one prop per bullet, each revealing progressively more content</li>
      </ul>
      <h4>Blank</h4>
      <p>A black/empty slide. Optionally add confidence monitor text (shown off-canvas, visible only to presenter). Toggle <strong>Blank Before</strong> on other slides to auto-insert a blank before them.</p>
      <h4>Image</h4>
      <p>A slide with a background image. The image is referenced from your ProPresenter media library.</p>
      <h4>Custom</h4>
      <p>Free-form slide for anything that doesn't fit the above types.</p>
    `,
  },
  {
    id: 'bible',
    ...
    html: `
      <h3>Bible Lookup</h3>
      <p>Scripture slides include a lookup tool powered by API.Bible. Enter your API key once in <strong>Presentation Settings</strong> (gear icon) — it's saved and hidden like a password.</p>
      <h4>Using It</h4>
      <ul>
        ...
        <li>The body text and reference bar auto-fill from the API response</li>
      </ul>
      <h4>Translations</h4>
      <ul>
        <li>Set a <strong>Default Translation</strong> in settings (NLT recommended for Canvas)</li>
        <li>Override per-slide using the translation picker next to the reference field</li>
        <li>Your subscribed translations appear in settings; public translations are also available</li>
      </ul>
    `,
  },
  {
    ...
    html: `
      ...
      <p>The panel on the right side has two tabs:</p>
      <h4>Outline Tab</h4>
      <p>Shows a live list of all slides with their content. Useful for getting a bird's-eye view of the full message flow while editing individual slides.</p>
      <h4>Speaker Notes Tab</h4>
      <p>Attach a script or notes document for reference while building the deck. Two ways to load:</p>
      <ul>
        <li><strong>Drop a PDF</strong> — drag any PDF file onto the drop zone</li>
        <li><strong>Google Drive link</strong> — paste a share link from Google Docs or Drive (file must be shared "Anyone with the link can view")</li>
      </ul>
      <p>Use the zoom controls to scale the PDF. Drag the left edge of the panel to resize it.</p>
      <p>Toggle the panel with the <strong>Outline</strong> button in the top-right of the main area.</p>
    `,
  },
  {
    id: 'generate',
    label: 'Generating',
    html: `
      <h3>Generating the Deck</h3>
      <p>Click <strong>Generate</strong> in the top-right header. A summary modal shows slide counts, file size, and type breakdown before confirming.</p>
      <h4>Output Modes</h4>
      <ul>
        <li><strong>Save to folder</strong> — writes the .pro file directly into your ProPresenter library folder</li>
        <li><strong>Download</strong> — delivers the file as a browser download (useful if running the server remotely)</li>
        <li><strong>Ask every time</strong> — prompts each time you generate</li>
      </ul>
      <p>The output folder and mode are set in <strong>Presentation Settings</strong> (gear icon).</p>
      <h4>What Gets Generated</h4>
      <ul>
        <li>A <code>Message_YY.MM.DD_Series_Title.pro</code> presentation file</li>
        <li>A matching <code>_Props.pro</code> file with all slide props</li>
      </ul>
    `,
  },
  {
    ...
    html: `
      ...
      <p>DeckPro can connect to a running instance of ProPresenter 7 on your local network to read live macro and prop data.</p>
      <h4>Setup</h4>
      <ul>
        ...
        <li>Enter the same port and password in DeckPro's <strong>Presentation Settings</strong></li>
      </ul>
      <p>The status dot in the header shows green when connected. DeckPro uses the connection to verify macro UUIDs and available props.</p>
    `,
  },
];
...
// Background: check Pro7 status and load fonts
checkPro7(true);
loadFonts();
